...
const OBTENER_USUARIO = gql`
  query ($input: FindUserByEmailInput!) {
    user(findUserByEmailInput: $input) {
      id
    }
  }
`;
...
export default function IntegrantesCreateScreen({ route }) {
  ...
  const [getUser, { loading, error, data, refetch }] =
    useLazyQuery(OBTENER_USUARIO);
  const navigation = useNavigation();
  const [modalVisible, setModalVisible] = useState(false);
  const showModal = () => setModalVisible(true);
  const hideModal = () => setModalVisible(false);

  const [newIntegrante, { loading: cargando, error: errores }] =
    useMutation(CREAR_INTEGRANTE);
